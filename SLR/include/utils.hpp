...
template <typename it> void printf_tuple(const ::std::tuple<it> &) {
...
template <typename it> void printf_generation(const generation<it> &) {
  ::std::cout << typeid(it).name() << std::endl;
}
...
void print_closure(const ::std::tuple<> &) { ::std::endl(::std::cout); }
